...
Le code doit contenir un README.md expliquant la **démarche de reproductibilité** à suivre. **Si vous avez utilisé une IA générative, vous devez préciser dans le fichier README.md le nom du modèle, l'usage qui en a été fait (pourquoi et comment), ainsi que quelques exemples de prompts utilisés.**
